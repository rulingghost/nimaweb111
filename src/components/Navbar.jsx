...
import { Menu, X, ChevronDown, MessageSquarePlus, Sun, Moon, Globe } from 'lucide-react';
...
export default function Navbar({ onOpenProposal }) {
  ...
  const handleSectorClick = (e, path) => {
    setIsOpen(false);
  };
      ...
      <header className={`navbar-wrapper ${scrolled ? 'scrolled' : ''}`}>
        <div className="navbar-inner">
          ...
          <div className="desktop-menu">
            ...
            <Link 
              to="/hakkimizda" 
              className={`nav-link ${location.pathname === '/hakkimizda' ? 'active' : ''}`}
            >
              {t('nav_about')}
            </Link>
                ...
                {localizedSectors.map((sector) => {
                  ...
                  return (
                    <Link 
                      ...
                      onClick={(e) => handleSectorClick(e, sector.path)}
                    >
      ...
      <div className={`mobile-menu ${isOpen ? 'open' : ''}`}>
        <div className="mobile-menu-inner">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Link to="/" className="mobile-nav-link" onClick={() => setIsOpen(false)}>{t('nav_home')}</Link>
          ...
          <Link to="/hakkimizda" className="mobile-nav-link" onClick={() => setIsOpen(false)}>{t('nav_about')}</Link>
          ...
          <Link to="/iletisim" className="mobile-nav-link" onClick={() => setIsOpen(false)}>{t('nav_contact')}</Link>
